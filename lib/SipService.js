...
function SipService(config, conferenceService) {
  const self = this;
  self.config = config;
      ...
      if (conference.sip) {
        self.register(conference.sip, function(response) {
          if (response && response.success && response.registration) {
            ...
            console.log('registered id ' + conference.sip.registrationId + " for conference " + conference.id);
//            self.subSocket.subscribe(conference.sip.registrationId);
          }
        });
      }
    });
...
SipService.prototype.sendMessage = function(message, destination) {
  const self = this;
  console.log('SipService.sendMessage: ' + JSON.stringify(message));
...
//  console.log('handleWebRtc', msg);
  if (msg.sessionOffer) {
        ...
        if (conference.sipConferenceId) {
          ...
          self.sendRequest(request, call.uuid, "ZMQ2SIP", function(response) {
            ...
            if (response && response.success) {
              console.log('call ' + call.uuid + ' has joined conference ' + conference.id);
            } 
          });
        } else {
    ...
    const call = self.sipCalls[uuid];
    if (call && conference) {
      call.member = { webrtc: false, uuid: Uuid.v4(), audio: true, video: false, moderator: false, streams: { } };
      ...
      const stream = { name: 'telephone', audio: true, video: false, uuid: Uuid.v4(), rxRtpEndpointId: rtpEndpointId };
