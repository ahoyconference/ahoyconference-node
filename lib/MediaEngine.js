...
MediaEngine.prototype.createRtpEndpoint = function(options, callback) {
  ...
  const message = {
    createRtpEndpointRequest: {
      ...
      loopback: (options.loopback !== undefined)?options.looback:false,
      iceInlineCandidates: true,
      uuid: uuid
    }
  };
  self.sendRequest(message, uuid, callback);
...
MediaEngine.prototype.createRtpEndpointFromOffer = function(options, callback) {
  const self = this;
  const uuid = Uuid.v4();
  const message = {
    createRtpEndpointRequest: {
      ...
      pcap: { basename: uuid + "_rtp" },
      decodeAudio: ((options.voiceActivityDetectionMode >= 0)?true:false),
      transparentRtcp: true,
      rtcpCheating: options.rtcpCheating,
      voiceActivityDetectionMode: options.voiceActivityDetectionMode,
      loopback: (options.loopback !== undefined)?options.loopback:false,
      iceInlineCandidates: true,
      uuid: uuid
    }
  };
...
MediaEngine.prototype.setRtpEndpointMediaFilter = function(rtpEndpointId, options, callback) {
  const self = this;
  const uuid = Uuid.v4();
  const message = {
    setRtpEndpointMediaFilterRequest: {
      id: rtpEndpointId,
      transmitSubscribedAudio: options.transmitSubscribedAudio,
      transmitSubscribedVideo: options.transmitSubscribedVideo,
      publishReceivedAudio: options.publishReceivedAudio,
      publishReceivedVideo: options.publishReceivedVideo,
      uuid: uuid
    }
  };
  if (options.transmitVideoStreamId) {
    message.setRtpEndpointMediaFilterRequest.transmitVideoStreamId = options.transmitVideoStreamId;
  }
  self.sendRequest(message, uuid, callback);
}
