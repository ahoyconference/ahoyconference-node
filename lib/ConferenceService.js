...
ConferenceService.prototype.handleJoinConferenceRequest = function(msg, client) {
  ...
  if (self.conferences[msg.conferenceId]) {
    ...
    client.send('joinConferenceResponse', { success: true, memberList: conference.members, member: client.member, mode: conference.mode, options: conference.options, locked: conference.locked, recordings: conference.recordings });
  ...
  client.send = function(message, data) {
    try {
      if (message === 'joinConferenceResponse') {
        client.socket.emit(message, data.success, data.memberList, data.member, data.mode, data.options, data.locked, data.recordings);
