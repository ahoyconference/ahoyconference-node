...
ConferenceService.prototype.onStreamStatus = function(stream, active, member) {
  ...
  if (conference) {
    ...
    self.io.to('conference_' + conference.id).emit('streamStatus', stream, active, member);
    if (member.webrtc) {
      self.forEachMemberStream(conference, false, function(sipStream) {
        if (sipStream.rtpMixerId) {
          if (active) {
            ...
            self.mediaEngine.removeRtpMixerSource(sipStream.rtpMixerId, stream.rxRtpEndpointId);
          }
        }
      });
    }
...
ConferenceService.prototype.onConnection = function(socket) {
  const self = this;
  const member = { webrtc: true, uuid: Uuid.v4(), moderator: false, streams: {} };
