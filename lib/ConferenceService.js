...
ConferenceService.prototype.onMemberLeave = function(conference, member) {
  ...
  console.log('member ' + member.name + '(' + member.uuid + ') left conference ' + conference.id + ', now ' + (Object.keys(conference.members).length - 1) + ' members');
  ...
  if (Object.keys(conference.members).length === 0) {
    conference.locked = false;
  }
}
...
ConferenceService.prototype.handleJoinConferenceRequest = function(msg, client) {
  ...
  if (self.conferences[msg.conferenceId]) {
    ...
    // send memberList with empty streams for old clients
    const memberList = {};
    var memberUuids = Object.keys(conference.members);
    memberUuids.forEach(function(memberUuid) {
      const member = conference.members[memberUuid];
      memberList[member.uuid] = {
        name: member.name,
        moderator: member.moderator,
        webrtc: member.webrtc,
        uuid: member.uuid,
        streams: {}
      };
    });
    client.send('joinConferenceResponse', { success: true, memberList: memberList, member: client.member, mode: conference.mode, options: conference.options, locked: conference.locked, recordings: conference.recordings, conferenceId: conference.id });
    ...
    // send a streamStatus event for each active stream to trigger subscriptions
    memberUuids = Object.keys(conference.members);
    memberUuids.forEach(function(memberUuid) {
      const member = conference.members[memberUuid];
      const streamUuids = Object.keys(member.streams);
      streamUuids.forEach(function(streamUuid) {
        const stream = member.streams[streamUuid];
        if (stream && stream.active && (stream.audio || stream.video)) {
          client.send('streamStatus', { stream: stream, active: stream.active, member: member });
        }
      });
    });
  } else {
...
ConferenceService.prototype.handleUnsubscribeStream = function(msg, client) {
  const self = this;
  const conference = self.conferences[client.member.conferenceId];
  if (conference && msg.subscription) {
    console.log('ConferenceService.handleUnsubscribeStream: ' + JSON.stringify(msg.subscription));
    if (client.member.subscriptions[msg.subscription.uuid]) {
      const subscription = client.member.subscriptions[msg.subscription.uuid];
      if (subscription && subscription.txRtpEndpointId) {
        self.mediaEngine.destroyRtpEndpoint(subscription.txRtpEndpointId);
        delete client.member.subscriptions[msg.subscription.uuid];
      }
    }
  }
}
...
ConferenceService.prototype.handleConferenceRecordingStatus = function(msg, client) {
  const self = this;
  const conference = self.conferences[client.member.conferenceId];
  if (conference && conference.options && conference.options.recording && (client.member.moderator || conference.options.recording.allowGuestRecording)) {
    if (msg.recording) {
...
ConferenceService.prototype.handleMessage = function(message, client) {
  ...
  try {
    ...
    } else if (msg.unsubscribeStream) {
      self.handleUnsubscribeStream(msg.unsubscribeStream, client);
...
ConferenceService.prototype.onConnection = function(socket) {
  ...
  client.member.disconnect = function() {
    ...
    client.send('memberLeft', { member: client.member });
...
// remove endpoint it
  ...
  // a client wants to stop receiving a stream
  socket.on('unsubscribeStream', function(subscription) {
    self.handleMessage({ unsubscribeStream: { subscription: subscription } }, client);
  });
